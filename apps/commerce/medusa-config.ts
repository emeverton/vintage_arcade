...
import { checkoutPreviewEnabled } from "./src/domain/checkout-preview"

loadEnv(process.env.NODE_ENV || "development", process.cwd())
const env = readEnvironment(process.env)
sliceEnabled(process.env)
checkoutPreviewEnabled(process.env)
const ciCookies = isolatedCiCookieOptions(process.env)

module.exports = defineConfig({
  projectConfig: {
    databaseUrl: env.databaseUrl, redisUrl: env.redisUrl, workerMode: env.workerMode,
    ...(ciCookies ? { cookieOptions: ciCookies } : {}),
    http: { storeCors: env.storeCors, adminCors: env.adminCors, authCors: env.authCors, jwtSecret: env.jwtSecret, cookieSecret: env.cookieSecret },
  },
  admin: { disable: env.adminDisabled, backendUrl: process.env.MEDUSA_BACKEND_URL || "http://localhost:9000" },
  modules: [
    { resolve: "@medusajs/medusa/event-bus-redis", options: { redisUrl: env.redisUrl, queueName: "vintage-events", jobOptions: { attempts: 5, backoff: { type: "exponential", delay: 1000 }, removeOnComplete: { age: 3600, count: 1000 }, removeOnFail: { age: 604800, count: 10000 } } } },
    ...
    { resolve: "@medusajs/medusa/fulfillment", options: { providers: [ { resolve: "@medusajs/medusa/fulfillment-manual", id: "manual" }, { resolve: "./src/modules/vintage-fulfillment", id: "vintage" } ] } },
    { resolve: "./src/modules/vintage-delivery" },
    { resolve: "./src/modules/vintage-food" },
    { resolve: "./src/modules/vintage-telemetry" },
    { resolve: "./src/modules/vintage-checkout" },
  ],
})
