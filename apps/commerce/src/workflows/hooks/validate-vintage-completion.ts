...
completeCartWorkflow.hooks.validate(async ({ cart }, { container }) => {
  ...
  for (const method of cart.shipping_methods || []) {
    ...
    const quote = evaluateDeliveryPolicy(option.data?.vintage_policy, cart as unknown as PricingCart)
  ...
  for (const command of commands) {
    const items = (cart.items || []).filter((i) => i.metadata?.vintage_command_id === command.id)
    if (!items.length) continue // The whole combo may be removed.
