...
/** All prices, addresses, products and thresholds below are synthetic QA fixtures. */
export default async function smokeCommerce({ container }: ExecArgs) {
  ...
  assert.equal(medusaToMinor(cart.item_total), 2500)
  ...
  let q = evaluateDeliveryPolicy(policy, cart)
  assert.equal(q.gapMinor, 500)
  ...
  assert.equal(medusaToMinor(cart.shipping_total), 900)
  assert.equal(medusaToMinor(cart.total), 3400)
  ...
  cart = await readCart(container, cartId)
  q = evaluateDeliveryPolicy(policy, cart)
  assert.equal(q.status, "free_shipping")
  assert.equal(medusaToMinor(cart.shipping_total), 0)
  assert.equal(medusaToMinor(cart.total), 3000)
  ...
  assert.equal(medusaToMinor(order.total), 3000)
  assert.equal(medusaToMinor(order.shipping_total), 0)
  assert.equal(order.items?.length, 3)
  assert.equal(order.items?.filter((i) => i.metadata?.vintage_combo_id === combo.id).length, 2)
  ...
  assert.equal(ledger[0].event_name, "order_created")
  assert.equal(ledger[0].payload.value_minor, 3000)
  pass("native_event_consumed_persisted_and_duplicate_delivery_deduplicated")
  // Deliberately corrupt only a synthetic shipping row to prove the completion guard.
  ...
  writeFileSync(".cache/commerce-slice-report.json", JSON.stringify(report, null, 2) + "\n")
