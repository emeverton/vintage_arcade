/** Storefront copy of staging URL/auth guards. Keep aligned with apps/commerce/src/domain/staging-runtime.ts. */
...
export function authorizeStagingBasic(header: string | null | undefined, env: Record<string, string | undefined>): boolean {
  ...
  if (user.length < 3 || password.length < 16) return false
  if (!header || !header.startsWith("Basic ")) return false
  let decoded = ""
  try { decoded = Buffer.from(header.slice(6), "base64").toString("utf8") } catch { return false }
  const separator = decoded.indexOf(":")
  if (separator < 1) return false
  return timingSafeEqualText(decoded.slice(0, separator), user) && timingSafeEqualText(decoded.slice(separator + 1), password)
}
