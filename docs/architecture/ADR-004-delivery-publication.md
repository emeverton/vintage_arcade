...
Data: 20/09/2026. Escopo: incremento 1C em local/test. Implementação candidata até concluir o CI.
...
A API exige usuário Medusa autenticado e existente, além de uma permissão explícita do servidor. viewer consulta e simula; editor também salva rascunhos; publisher também publica e reverte. Listas de IDs ficam apenas no ambiente do backend e vazias por padrão. Nenhum usuário real recebe acesso automaticamente. A gestão dinâmica dessas permissões e o RBAC integral do admin permanecem fora deste incremento.

As mutações verificam Content-Type e Origin, inclusive para prevenir alteração via sessão de origem não aprovada. O corpo tem limite de 16 KB. Durante a homologação, mutações nas rotas nativas de ShippingOption são bloqueadas para impedir contornar o fluxo auditado. Provisionamento inicial continua por workflow interno.
...
O carregamento de políticas é limitado a 500 vínculos neste estágio de loja única e falha explicitamente acima disso. O backoffice pagina opções de entrega e sinaliza truncamento após 100 revisões ou registros de auditoria. Escalonamento exige consultas por escopo e paginação completa, não leitura ilimitada.
